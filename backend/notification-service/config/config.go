...
var Config = struct {
	...
	Kafka struct {
		...
		Enable           bool   `default:"true" yaml:"enable" env:"KAFKA_ENABLED"`
	}
	SmsAPI struct {
		...
		UserName        string `env:"SMPP_USER_NAME"`
		Password        string `env:"SMPP_PASSWORD"`
		Source          string `env:"SMPP_SOURCE"`
		EntityId        string `env:"SMPP_ENTITY_ID"`
	}
	EmailSMTP struct {
