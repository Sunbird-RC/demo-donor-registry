...
export class AttestationComponent implements OnInit {
    ...
    this.generalService.getData(this.apiUrl).subscribe((res) => {
      ...
      if (res.claim) {
        ...
        this.schemaService.getFormJSON().subscribe((FormSchemas) => {
          ...
          let _self = this;
         // let tempO = {};
          Object.keys(_self.attestationData).forEach(function (key) {
            if (typeof (_self.attestationData[key]) == 'object') {
            
            //  Object.assign(tempO, _self.attestationData[key])
              _self.attestationData =  _self.attestationData['personalDetails'];
  ...
  onAttestApproveReject(action, event) {
    ...
    this.generalService.postData(url, data).subscribe((res) => {
      this.toastMsg.success('Success', "Application " + actionName + " Successfully");
      ...
      (err) => {
        this.toastMsg.error('Success', err.error.params.errmsg);
      });
